...
void eth_force_link_speed(u32 *supported)
{
	const char *str_env;
	u32 required;
	...
	str_env = env_get("eth_force_link");
	if (!str_env)
		return;

	/* Code to force a specific link speed in U-Boot */
	if (!strcmp(str_env, "10baseT_Full") &&
	    (*supported & SUPPORTED_10baseT_Full))
		required |= SUPPORTED_10baseT_Full;
	else if (!strcmp(str_env, "10baseT_Half") &&
		 (*supported & SUPPORTED_10baseT_Half))
		required |= SUPPORTED_10baseT_Half;
	else if (!strcmp(str_env, "100baseT_Full") &&
		 (*supported & SUPPORTED_100baseT_Full))
		required |= SUPPORTED_100baseT_Full;
	else if (!strcmp(str_env, "100baseT_Half") &&
		 (*supported & SUPPORTED_100baseT_Half))
		required |= SUPPORTED_100baseT_Half;
	else if (!strcmp(str_env, "1000baseT_Full") &&
